...
• Auth is required before commands`
	...
	scTypeSelect.OnChanged = func(scType string) {
		...
		// CXRF mode setup wizard
		if scType == "CXRF" {
			if portSelect.Selected == "" {
				dialog.ShowError(fmt.Errorf("please select a serial port first"), myWindow)
				scTypeSelect.SetSelected(previousSCType)
				return
			}

			// Step 1: Authenticate and check EEP 3961 01
			go func() {
				// Display warning about EEPROM checksum
				timestamp := time.Now().Format("15:04:05")
				fyne.Do(func() {
					outputText.SetText(outputText.Text + fmt.Sprintf("\n[%s] ══════════════════════════════════════════════════════\n", timestamp) +
						"  WARNING: ENTERING CXRF MODE\n" +
						"  \n" +
						"  Setting EEP offset 3961 will temporarily prevent the PS3\n" +
						"  from booting when turned off then on again, as this\n" +
						"  invalidates the EEPROM checksum for the syscon.\n" +
						"  \n" +
						"  This will be fixed when the correct checksum is validated.\n" +
						"══════════════════════════════════════════════════════════\n\n")
				})

				// Authenticate using CXR mode (external commands)
				ps3, err := NewPS3UART(portSelect.Selected, "CXR", 57600)
				if err != nil {
					fyne.Do(func() {
						dialog.ShowError(fmt.Errorf("failed to open serial port: %v", err), myWindow)
						scTypeSelect.SetSelected(previousSCType)
					})
					return
				}
				...
				authResult := ps3.Auth()
				fyne.Do(func() {
					outputText.SetText(outputText.Text + fmt.Sprintf("[%s] > AUTH: %s\n", time.Now().Format("15:04:05"), authResult))
				})

				// Check for RX/TX swap error
				if strings.Contains(authResult, "Auth1 response invalid") {
					ps3.Close()
					fyne.Do(func() {
						outputText.SetText(outputText.Text + "[ERROR] Auth1 response invalid - RX/TX wires may be swapped\n")
						dialog.ShowError(fmt.Errorf("auth1 response invalid - please swap the RX and TX wires and try again"), myWindow)
						scTypeSelect.SetSelected(previousSCType)
					})
					return
				}
				...
				if !strings.Contains(authResult, "Auth successful") {
					ps3.Close()
					fyne.Do(func() {
						outputText.SetText(outputText.Text + "[ERROR] Authentication failed\n")
						dialog.ShowError(fmt.Errorf("authentication failed: %s", authResult), myWindow)
						scTypeSelect.SetSelected(previousSCType)
					})
					return
				}

				// Send EEP GET 3961 01
				result := ps3.Command("EEP GET 3961 01", 1)
				fyne.Do(func() {
					outputText.SetText(outputText.Text + fmt.Sprintf("[%s] > EEP GET 3961 01: %08X %s\n", time.Now().Format("15:04:05"), result.Code, strings.Join(result.Data, " ")))
				})

				// Check if result is 00000000 00
				needsSet := true
				if result.Code == 0x00000000 && len(result.Data) > 0 && result.Data[0] == "00" {
					needsSet = false
				}

				if needsSet {
					// Send EEP SET 3961 01 00
					setResult := ps3.Command("EEP SET 3961 01 00", 1)
					fyne.Do(func() {
						outputText.SetText(outputText.Text + fmt.Sprintf("[%s] > EEP SET 3961 01 00: %08X %s\n", time.Now().Format("15:04:05"), setResult.Code, strings.Join(setResult.Data, " ")))
					})
					if setResult.Code != 0x00000000 {
						ps3.Close()
						fyne.Do(func() {
							dialog.ShowError(fmt.Errorf("failed to set EEP 3961 01 00"), myWindow)
							scTypeSelect.SetSelected(previousSCType)
						})
						return
					}
				...
				// Step 2: Show popup - turn off console
				fyne.Do(func() {
					dialog.ShowInformation("CXRF Setup - Step 1/3",
						"Please turn OFF the console now.\n\nThen ground the DIAG pin by connecting it to:\n• GND terminal, or\n• GND USB pin",
						myWindow)
				})

				// Step 3: Confirm DIAG pin grounded
				fyne.Do(func() {
					dialog.ShowConfirm("CXRF Setup - Step 2/3",
						"Have you grounded the DIAG pin?",
						func(confirmed bool) {
							if !confirmed {
								scTypeSelect.SetSelected(previousSCType)
								return
							...
							// Step 4: Turn console back on
							dialog.ShowInformation("CXRF Setup - Step 3/3",
								"Please turn the console back ON now.\n\nWaiting for console to boot (30 seconds)...",
								myWindow)

							// Step 5: Wait 30 seconds then attempt auth
							go func() {
								time.Sleep(30 * time.Second)

								// Silently attempt auth until response
								var authResponse string
								authSuccess := false
								for attempts := 0; attempts < 60; attempts++ {
									ps3, err := NewPS3UART(portSelect.Selected, "CXRF", 115200)
									if err != nil {
										time.Sleep(1 * time.Second)
										continue
									}

									authResponse = ps3.Auth()
									ps3.Close()

									// Check for RX/TX swap error
									if strings.Contains(authResponse, "Auth1 response invalid") {
										fyne.Do(func() {
											outputText.SetText(outputText.Text + "[ERROR] Auth1 response invalid - RX/TX wires may be swapped\n")
											dialog.ShowError(fmt.Errorf("auth1 response invalid - please swap the RX and TX wires and try again"), myWindow)
											scTypeSelect.SetSelected(previousSCType)
									...
									if strings.Contains(authResponse, "Auth successful") {
										authSuccess = true
										break
									}
									time.Sleep(1 * time.Second)
								...
								if !authSuccess {
									fyne.Do(func() {
										outputText.SetText(outputText.Text + "[ERROR] Unable to authenticate after 60 attempts\n")
										dialog.ShowError(fmt.Errorf("unable to authenticate - please check your connections and try again"), myWindow)
										scTypeSelect.SetSelected(previousSCType)
									})
									return
								}

								// Display the response and switch UI
								fyne.Do(func() {
									outputText.SetText(outputText.Text + fmt.Sprintf("[%s] > CXRF AUTH: %s\n", time.Now().Format("15:04:05"), authResponse))
									outputText.SetText(outputText.Text + "[SUCCESS] CXRF mode ready - internal commands available\n")
									dialog.ShowInformation("CXRF Mode Ready", "Successfully connected in CXRF mode.\n\nInternal commands are now available.", myWindow)

									// Switch to CXRF UI
									previousSCType = scType
